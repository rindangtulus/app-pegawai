...
<div class="px-6 py-24 sm:py-32 lg:px-8">
    ...
    <div class="mx-auto max-w-2xl text-center">
        <h2 class="text-4xl font-semibold tracking-tight text-balance text-gray-900 sm:text-5xl">Tambah Absensi Baru</h2>
        <p class="mt-2 text-lg/8 text-gray-600">Masukkan data absensi karyawan.</p>
    </div>

    <form action="{{ route('attendances.store') }}" method="POST" class="mx-auto mt-16 max-w-xl sm:mt-20">
        @csrf
        <div class="grid grid-cols-1 gap-x-8 gap-y-6 sm:grid-cols-2">
            ...
            <div>
                ...
                <div class="mt-2.5">
                    <input id="tanggal" type="date" name="tanggal" value="{{ old('tanggal') }}" class="block w-full rounded-md bg-white px-3.5 py-2 text-base text-gray-900 outline-1 -outline-offset-1 outline-gray-300 placeholder:text-gray-400 focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-600" />
                </div>
                @error('tanggal')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <div class="mt-2.5">
                    <input id="waktu_masuk" type="time" name="waktu_masuk" value="{{ old('waktu_masuk') }}" class="block w-full rounded-md bg-white px-3.5 py-2 text-base text-gray-900 outline-1 -outline-offset-1 outline-gray-300 placeholder:text-gray-400 focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-600" />
                </div>
                @error('waktu_masuk')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <div class="mt-2.5">
                    <input id="waktu_keluar" type="time" name="waktu_keluar" value="{{ old('waktu_keluar') }}" class="block w-full rounded-md bg-white px-3.5 py-2 text-base text-gray-900 outline-1 -outline-offset-1 outline-gray-300 placeholder:text-gray-400 focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-600" />
                </div>
                @error('waktu_keluar')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
